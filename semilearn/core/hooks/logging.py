            ...
            if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
                ...
                for i, (key, item) in enumerate(algorithm.log_dict.items()):
                    ...
                    if i != len(algorithm.log_dict) - 1:
                        ...
                        print_text += " "
                algorithm.print_fn(print_text)
                if algorithm.algorithm == 'main':
                    algorithm.print_fn('Additional logging info: {}: {}, {}: {}, {}: {}, {}: {} {}: {}'.format('train/ulb_pseudo_label_acc', algorithm.ulb_pseudo_label_acc, 'train/pseudo_class_util_ratio',
                                                                                                        algorithm.pseudo_class_util_ratio, 'train/real_class_util_ratio', algorithm.real_class_util_ratio,
                                                                                                        'train/class_ulb_pseudo_label_acc', algorithm.class_ulb_pseudo_label_acc,
                                                                                                        'train/class_ulb_pseudo_label_precision', algorithm.class_ulb_pseudo_label_precision))