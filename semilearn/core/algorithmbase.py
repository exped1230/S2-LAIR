...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        #self.model, self.diffusion_model = self.set_model()
    ...
    def set_optimizer(self):
        ...
        elif self.args.scheduler == 'lr':
            from torch.optim import lr_scheduler
            scheduler = lr_scheduler.StepLR(optimizer, int(self.num_train_iter // 5), gamma=0.1)
        else:
            print('the parameter of scheduler is wrong')
            exit(0)
    ...
    def set_hooks(self):
        ...
        # update loss, optimizer, and scheduler in after_train_step
        self.register_hook(ParamUpdateHook(), None, "HIGHEST")
        # define EMA in before_run, and update EMA model in after_train_step
        self.register_hook(EMAHook(), None, "HIGH")
        # eval after the n iterations(set in yaml) in after_train_step, and save model in after_run
        self.register_hook(EvaluationHook(), None, "HIGH")
        # save checkpoint (model) in every n iteration
        self.register_hook(CheckpointHook(), None, "HIGH")
        # update the value of epoch in Sampler 
        self.register_hook(DistSamplerSeedHook(), None, "NORMAL")
        # record the time and learning rate in before/after train step
        self.register_hook(TimerHook(), None, "LOW")
        # update the logging for eval and parameters of model
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for data_lb, data_ulb in zip(self.loader_dict['train_lb'],
                ...
                # process_batch可以自适应地选择train_step需要的参数，最多可以支持的参数包括有标签样本的序号、样本、标签，无标签样本的序号、弱增强实例、强增强实例
    ...
    def get_save_dict(self):
        ...
        save_dict = {
            ...
            #'diffusion_ema_model': self.diffusion_ema_model.state_dict(),
    ...
    def save_model(self, save_name, save_path):
        ...
        self.print_fn(f"model saved: {save_filename}, now the model is not saved, if you want to save, change the function save_model in algorithmbase.py")
    ...
    def load_model(self, load_path):
        ...
        #self.diffusion_ema_model.load_state_dict(checkpoint['diffusion_ema_model'])
